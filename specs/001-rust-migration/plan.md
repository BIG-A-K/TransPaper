...
- candle-core, candle-nn (HuggingFace candle, latest) — DocLayout-YOLO 推論
...
- PDF操作ライブラリ — PoC Phase 2 で選定（候補: mupdf-rs, lopdf, pdf-rs）
...
└── src/
    ...
    ├── seg.rs           # Segmentation (candle + DocLayout-YOLO)
...
| フェーズ | 内容 | Go/No-Go |
|----------|------|----------|
| PoC Phase 1 | PNG画像を入力として candle で DocLayout-YOLO 推論を実行。Python版の出力と目視比較 | **candle で推論不可なら中止** |
| PoC Phase 2 | Rust で PDF → PNG レンダリングを検証。PDF操作ライブラリを選定 | **PDF操作要件を満たせなければ中止** |
...
| spec要件 | 実装ファイル | 役割 |
|----------|--------------|------|
| DocLayout-YOLO の candle 推論 | `rust/src/seg.rs` | candle でモデルをロードし、画像に対してYOLO推論を実行。検出結果を SegmentBlock に変換 |
| DocLayout-YOLO の candle 推論 | `rust/src/model.rs` | モデルファイルの解決（ローカル優先→HuggingFace DL→フォールバック） |
...
| モデルの自動ダウンロード | `rust/src/model.rs` | hf-hub でHuggingFaceからsafetensors形式をダウンロード。キャッシュ管理 |
| 中間ファイル互換 | `rust/src/schema.rs` | SegmentPage, TranslatedPage 等をserde derive で定義。Python版JSON互換 |

## リスク・調査項目

- **candle で DocLayout-YOLO が動くか**: モデルアーキテクチャ（YOLOv10ベース）を candle で再現できるか未検証。safetensors への変換可否も含む。→ PoC Phase 1 で検証
- **PDF操作ライブラリの能力**: redaction（テキスト塗りつぶし）と TextWriter 相当の機能があるか。mupdf-rs は AGPL ライセンスの確認が必要。→ PoC Phase 2 で検証
- **クロスコンパイル**: candle の Apple Silicon / Linux x86_64 ビルドが stable で通るか。GPU 非依存（CPU推論のみ）を前提とする
...
- **段階的移行**: PoC Phase 1（candle検証）→ Phase 2（PDF検証）→ 各ステージ順次実装。PNG直接入力で最速Go/No-Go判断
